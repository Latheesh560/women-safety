...
import { Shield, User, Mail, Phone, Lock, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !email || !phone || !password) { setLocalError('All fields are required.'); return; }
  ...
  return (
    <div className="min-h-screen text-slate-800 py-12 px-4 sm:px-6 lg:px-8 flex flex-col justify-center items-center relative">
      ...
      <motion.div
        ...
        <h2 className="text-xl font-bold text-center text-slate-800 mb-2 tracking-tight">Create Secure Account</h2>
        <p className="text-xs text-center text-slate-500 mb-6 font-semibold">
          Sign up to access safe routes, live reporting, and fast-acting SOS services.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" name="name" required value={formData.name} onChange={handleChange} placeholder="Jane Doe" className="input-field pl-11 border-slate-200/80 bg-white/90 focus:bg-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="tel" name="phone" required value={formData.phone} onChange={handleChange} placeholder="9876543210" className="input-field pl-11 border-slate-200/80 bg-white/90 focus:bg-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} name="confirmPassword" required value={formData.confirmPassword} onChange={handleChange} placeholder="••••••••" className="input-field pl-11 border-slate-200/80 bg-white/90 focus:bg-white" />
            </div>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full btn-pink flex justify-center items-center py-3 bg-gradient-to-tr from-primary to-accent text-white border-0 font-bold shadow-md shadow-pink-100 hover:opacity-95 transition-all">
            ...
            ) : 'Register & Set Safe Route'}
          </button>
        </form>

        <p className="mt-6 text-sm text-center text-slate-500 font-semibold">
          Already have an account?{' '}
          <Link to="/login" className="font-bold text-primary-dark hover:text-accent transition-colors">Sign In here</Link>
        </p>
      </motion.div>
    </div>
  );
